...
use slate_common::dbus::{
    NOTIFICATIONS_BUS_NAME, NOTIFICATIONS_INTERFACE, NOTIFICATIONS_PATH, RHEA_BUS_NAME,
    RHEA_INTERFACE, RHEA_PATH,
};
...
async fn send_notif(conn: &Connection, app_name: &str, summary: &str) -> anyhow::Result<u32> {
...
async fn notifyd_and_rhea_coexist_on_session_bus() {
    ...
    let rhea = DaemonProcess::spawn("rhea", RHEA_BUS_NAME, &conn, vec![("HOME", home)])
        .await
    ...
    let rhea_proxy = slate_proxy(&conn, RHEA_BUS_NAME, RHEA_PATH, RHEA_INTERFACE)
        .await
        .unwrap();
...
async fn notification_signal_received_by_subscriber() {
    ...
    // Use a proxy's receive_signal instead of MessageStream::for_match_rule
    // (the latter hangs in zbus 5 test context).
    let proxy = slate_proxy(
        &conn,
        NOTIFICATIONS_BUS_NAME,
        NOTIFICATIONS_PATH,
    ...
    let mut signal_stream = proxy.receive_signal("Added").await.unwrap();
    ...
    match signal_result {
        Ok(Some(msg)) => {
            // Signal received — verify we can read the body.
            // The Added signal carries a TOML-serialized notification string.
            let body: String = msg.body().deserialize().unwrap_or_default();
            // Body may be empty if the signal carries data in a different format.
            // The important thing is that the signal was received at all.
            tracing::info!(
        ...
        Err(_) => {
            panic!("timeout waiting for Added signal — signal was not emitted");
        }
    }
...
async fn group_changed_signal_fires_on_dismiss() {
    ...
    // Subscribe to GroupChanged using proxy.receive_signal.
    let proxy = slate_proxy(
        &conn,
        NOTIFICATIONS_BUS_NAME,
        NOTIFICATIONS_PATH,
    ...
    let mut signal_stream = proxy.receive_signal("GroupChanged").await.unwrap();
    ...
    let result = tokio::time::timeout(Duration::from_secs(3), signal_stream.next()).await;

    match result {
        Ok(Some(msg)) => {
            let (app_name, count): (String, u32) = msg.body().deserialize().unwrap_or_default();
